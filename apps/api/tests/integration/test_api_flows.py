...
def test_public_shape_demo_auth_watchlist_backtest_and_portfolio(client: TestClient) -> None:
...
def test_registration_verification_is_single_use_and_forgot_is_generic(
    client: TestClient,
) -> None:
    ...
    assert created
    captured_message = json.loads(created[-1].read_text(encoding="utf-8"))
    token_match = re.search(r"token=([^\n]+)", str(captured_message["text"]))
    assert token_match
...
def test_normal_user_cannot_trigger_admin_job(client: TestClient) -> None:
